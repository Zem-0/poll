...
export function mergeMessage(message: Message, event: ChatEvent) {
  // Handle content specifically for planner messages if it comes as a chunk
  if (message.agent === "planner" && event.type === "message_chunk" && event.data.content !== undefined) {
    message.content = event.data.content; // Assign the full content for planner
    message.contentChunks = [event.data.content]; // Reset chunks
